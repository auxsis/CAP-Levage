...
class inherit_payment_transaction(models.Model):
    ...
    def _createEquipment(self, product, user_id):
        ...
        self.env.cr.execute("SELECT count(*) FROM critt_equipment_category WHERE name = %s", [category_id.name])
        ...
        if int(categories[0][0]) == 0:
            ...
            template = {
                ...
                'category_article_id': category_id.id
            }
            newCateg = categ.create(template)
            category_id = newCateg.id
            done += newCateg
        else:
            self.env.cr.execute("SELECT id FROM critt_equipment_category WHERE name = %s", [category_id.name])
        ...
        #if newEquip != -1:
        self.env.cr.execute("SELECT id FROM ir_attachment WHERE res_id = %s AND res_model = 'product.template'",
                            [product.product_tmpl_id.id])
        images = self.env.cr.fetchall()
        for item in images:
            image = self.env['ir.attachment'].browse(int(item[0]))
            image.copy(
                default={'res_id': newEquip.id, 'res_model': 'critt.equipment', 'res_name': product.name})
