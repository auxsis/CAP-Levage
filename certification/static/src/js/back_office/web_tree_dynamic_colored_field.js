odoo.define('web_tree_dynamic_colored_field', function (require) {
    ...
    ListRenderer.include({
        ...
        applyColorizeHelper: function ($td, nodeOptions, node, nodeAttribute, cssAttribute, ctx) {
            //console.log('applyColorizeHelper');
            if (nodeOptions[nodeAttribute]) {
                ...
                for (var i=0, len=colors.length; i<len; ++i) {
                    ...
                    if (py.evaluate(expression, ctx).toJSON()) {
                        ...
                        var txt = $td.html();
                        //console.log('txt: ' + txt);
                        //console.log('css: ' + cssAttribute + ' color ' + color);
                        if(txt.indexOf('<p') == -1){
                            var $p = $('<p>' + txt + '</p>');
                            $p.css(cssAttribute, color);
                            $p.css('height', '20px');
                            $p.css('display', 'inline-block');
                            $p.css('margin', 'auto');
                            $p.css('padding', '1px 8px 0px 8px');
                            $td.empty();
                            $td.append($p);
                        }else{
                            var $p = $td.find('p');
